...
function scrptTag()
{
	...
	for(var i=0;i<len;i++)
	{
		...
		scriptstring = scriptstring.replace(/<![CDATA[\s\S]+?]]>/g,""); // //to remove commented javascript <![CDATA //]]>               
			...
			var encodedensity=((encodenumbers*100)/indScriptLength);           // added extra to reduce false positive 
			encodedanddigitnumbers=(indNumLength) + (encodenumbers);
...
if(count_if){
	framework.extension.fireEvent('status_if', { data: {someData:gsbreq, highlitedText:'CNN'}});  //dyn hid ifr
	//console.log("tag ifr src sent: "+gsbreq);
	s2='Threat:: Hidden Iframe<br>Evil URL(s) are :::'+alert_if+"<br>";
}

var gsbreq1=count_sc+alert_sc;
if(count_sc){
	framework.extension.fireEvent('status_sc', { data: {someData:gsbreq1, highlitedText:'CNN'}});  //dyn hid ifr
	//console.log("tag ifr src sent: "+gsbreq);
...
if(ps == null || ps.match(HostProto) == null){     //iniatiall null and host diffrenet so block
	...
	{       
		if(alert_if && frameEl === null){
			//console.log("in alert: "+alert_if);
	
			var blstatus_if="";
			framework.extension.attachEvent('ifgsbstat', function(e){
			blstatus_if=e.data.someData;
				...
				if(blstatus_if.match("malware") || blstatus_if.match("phising") || blstatus_if.match("phishing,malware")){
					flag=1;
					//console.log("and here is an alert");
		...
		if(alert_sc && flag == 0){
			var blstatus_sc="";
			framework.extension.attachEvent('scgsbstat', function(e){
			blstatus_sc=e.data.someData;
				//console.log("if blstatus rcvd"+blstatus_if);
...
var x=[];
 ...
 //sending data(s) to panel in chrome 

framework.extension.fireEvent('BIGHI', { data: {someData:x, highlitedText:'CNN'}});
